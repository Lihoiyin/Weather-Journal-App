...
const myKey = ',us&appid=954a621bd74eb250eb3ffa2848d0b434&units=metric';
...
buttonOfGernerate.addEventListener('click', (event)=>{/* Function called by event listener */
    ...
    .then((data) => {
        ...
        postData('/addData', {temp: data.main.temp, date: myDate, feelings: feelings });
...
const apiData = async(baseUrl, zip, myKey) => {
    const apiRes = await fetch(`${baseUrl}?q=${zip}${myKey}`);
    try {
